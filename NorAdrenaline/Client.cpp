...
void CL_CreateMove(float frametime, struct usercmd_s *cmd, int active)
{
	...
	if (g_Local.bAlive)
	{
		...
		g_Systems.AutoStrafe(cmd, frametime);
//		g_Systems.JumpBug(frametime, cmd);
	//	g_Systems.StrafeEmulator();
	//	g_Systems.StrafeHelper(frametime, cmd);
		//g_Systems.SimulateMouse();
		g_AimBot.Run(cmd);
...
/*int CL_IsCustomFov(void)
{
	if (!cvar.hide_from_obs && !g_pGlobals.bSnapshot && !g_pGlobals.bScreenshot && cvar.custom_fov > 0 && g_Local.bAlive)
		return 1;

	return g_Client.CL_IsCustomFov();
}*/
...
int HUD_AddEntity(int type, struct cl_entity_s *ent, const char *modelname)
{
	if (!cvar.hide_from_obs && cvar.esp && g_Utils.IsPlayer(ent) && g_Player[ent->index].bAlive && !g_pGlobals.bSnapshot && !g_pGlobals.bScreenshot)
	{
		...
		if (cvar.crosshair_Active)
		{
			byte r, g, b, a = 255;

			g_pEngine->pfnCvar_Set("cl_crosshair_color", "0");
			g_pEngine->pfnCvar_Set("cl_crosshair_translucent", "1");
			...
			int x = g_Screen.iWidth / 2;
			int y = g_Screen.iHeight / 2;

			if (cvar.crosshair_Outline)
			{
				g_Drawing.DrawCrosshair(x - 1, y, 0, 0, 0, a);
				g_Drawing.DrawCrosshair(x, y - 1, 0, 0, 0, a);
				g_Drawing.DrawCrosshair(x + 1, y, 0, 0, 0, a);
				g_Drawing.DrawCrosshair(x, y + 1, 0, 0, 0, a);

				g_Drawing.DrawCrosshair(x - 1, y - 1, 0, 0, 0, a);
				g_Drawing.DrawCrosshair(x - 1, y + 1, 0, 0, 0, a);
				g_Drawing.DrawCrosshair(x + 1, y - 1, 0, 0, 0, a);
				g_Drawing.DrawCrosshair(x + 1, y + 1, 0, 0, 0, a);
			}
			g_Drawing.DrawCrosshair(x, y, r, g, b, 255);

			if (cvar.crosshair_Dot)
			{
				if (cvar.crosshair_Outline)
				{
					g_Drawing.DrawDot(x - 1, y, 0, 0, 0, a);
					g_Drawing.DrawDot(x, y - 1, 0, 0, 0, a);
					g_Drawing.DrawDot(x + 1, y, 0, 0, 0, a);
					g_Drawing.DrawDot(x, y + 1, 0, 0, 0, a);

					g_Drawing.DrawDot(x - 1, y - 1, 0, 0, 0, a);
					g_Drawing.DrawDot(x + 1, y - 1, 0, 0, 0, a);
					g_Drawing.DrawDot(x - 1, y + 1, 0, 0, 0, a);
					g_Drawing.DrawDot(x + 1, y + 1, 0, 0, 0, a);
				}
				g_Drawing.DrawDot(x, y, r, g, b, a);
			}
		}
		else
		{
			g_pEngine->pfnCvar_Set("cl_crosshair_translucent", "0");
		}
...
void HookClient()
{
	...
	// g_pClient->CL_IsCustomFov = CL_IsCustomFov;
